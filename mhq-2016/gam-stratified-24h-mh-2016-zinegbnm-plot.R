...
m_phq_gam_sub_sleep_q1_zinegbnm <- readRDS(paste0(outputdir, "m_phq_gam_sub_sleep_q1_zinegbnm", ".RDS"))
m_phq_gam_sub_sleep_q2_zinegbnm <- readRDS(paste0(outputdir, "m_phq_gam_sub_sleep_q2_zinegbnm", ".RDS"))
m_phq_gam_sub_sleep_q3_zinegbnm <- readRDS(paste0(outputdir, "m_phq_gam_sub_sleep_q3_zinegbnm", ".RDS"))

m_gad_gam_sub_sleep_q1_zinegbnm <- readRDS(paste0(outputdir, "m_gad_gam_sub_sleep_q1_zinegbnm", ".RDS"))
m_gad_gam_sub_sleep_q2_zinegbnm <- readRDS(paste0(outputdir, "m_gad_gam_sub_sleep_q2_zinegbnm", ".RDS"))
m_gad_gam_sub_sleep_q3_zinegbnm <- readRDS(paste0(outputdir, "m_gad_gam_sub_sleep_q3_zinegbnm", ".RDS"))

# prep data ------------------
sub_models_resp   <- c("m_phq_gam_sub_sleep",
                       "m_gad_gam_sub_sleep"
)
sleep_type <- c("_q1_zinegbnm", "_q2_zinegbnm", "_q3_zinegbnm")

sub_models_zinegbnm <- list()
...
for(i in seq_along(sub_models_resp)) {
  ...
  sub_models_zinegbnm[[i]] <- sub_models_sleepg_type_i
}
...
# phq by sleep period----------------
sub_models_phq <- grep("phq", names(sub_models_zinegbnm), value = T)
phq            <- "Depressive Symptoms"
rg_phq <- expand.grid.df(data.frame(sub_models_phq, phq), 
                         data.frame(parts, part_labels))

phq_24h_zinegbnm <- foreach(i = seq_len(nrow(rg_phq)),
                            .packages = "multilevelcoda") %dopar% {
                              
                              ggplot(sub_models_zinegbnm[[rg_phq[i, "sub_models_phq"]]][To == eval(rg_phq[i, "parts"])], aes(x = Delta, y = Mean)) +
...
names(phq_24h_zinegbnm) <- foreach(i = seq_len(nrow(rg_phq))) %dopar% {
  paste0("Reallocation of Time between ", rg_phq[i, "part_labels"], " and ", rg_phq[i, "phq"])
}
phq_24h_zinegbnm

saveRDS(phq_24h_zinegbnm, paste0(outputdir, "phq_24h_zinegbnm", ".RDS"))
...
figure <- ggarrange(phq_24h_zinegbnm[[1]], phq_24h_zinegbnm[[2]], 
                    phq_24h_zinegbnm[[3]], phq_24h_zinegbnm[[4]], 
                    nrow = 4,
                    common.legend = TRUE,
                    legend   = "bottom"
)
...
# gad by sleep period----------------
sub_models_gad <- grep("gad", names(sub_models_zinegbnm), value = T)
gad            <- "Depressive Symptoms"
rg_gad <- expand.grid.df(data.frame(sub_models_gad, gad), 
                         data.frame(parts, part_labels))

gad_24h_zinegbnm <- foreach(i = seq_len(nrow(rg_gad)),
                            .packages = "multilevelcoda") %dopar% {
                              
                              ggplot(sub_models_zinegbnm[[rg_gad[i, "sub_models_gad"]]][To == eval(rg_gad[i, "parts"])], aes(x = Delta, y = Mean)) +
                                geom_hline(yintercept = 0, linewidth = 0.2, linetype = 2) +
                                geom_vline(xintercept = 0, linewidth = 0.2, linetype = 2) +
                                geom_ribbon(aes(ymin = CI_low,
                                                ymax = CI_high, fill = SleepPeriod),
...
names(gad_24h_zinegbnm) <- foreach(i = seq_len(nrow(rg_gad))) %dopar% {
  paste0("Reallocation of Time between ", rg_gad[i, "part_labels"], " and ", rg_gad[i, "gad"])
}
gad_24h_zinegbnm

saveRDS(gad_24h_zinegbnm, paste0(outputdir, "gad_24h_zinegbnm", ".RDS"))
...
figure <- ggarrange(gad_24h_zinegbnm[[1]], gad_24h_zinegbnm[[2]], 
                    gad_24h_zinegbnm[[3]], gad_24h_zinegbnm[[4]], 
                    nrow = 4,
                    common.legend = TRUE,
                    legend   = "bottom"
)
