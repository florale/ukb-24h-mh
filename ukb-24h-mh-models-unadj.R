source("ukb-24h-mh-data.R")

# PHQ9 -------------------
## all stratified by sleep quantiles ------------
m_unadj_phq_2023_gam_sleep_q1 <- brmcoda(clr_acc_mhq_sleep_q1,
                                         phq_2023 ~ 
                                           s(ilr1) + s(ilr2) + s(ilr3),
                                         family = zero_inflated_negbinomial(),
                                         chains = 4, cores = 4,
                                         backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q1, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q1", ".RDS"))

m_unadj_phq_2023_gam_sleep_q2 <- brmcoda(clr_acc_mhq_sleep_q2,
                                         phq_2023 ~ 
                                           s(ilr1) + s(ilr2) + s(ilr3),
                                         family = zero_inflated_negbinomial(),
                                         chains = 4, cores = 4,
                                         backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q2, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q2", ".RDS"))

m_unadj_phq_2023_gam_sleep_q3 <- brmcoda(clr_acc_mhq_sleep_q3,
                                         phq_2023 ~ 
                                           s(ilr1) + s(ilr2) + s(ilr3),
                                         family = zero_inflated_negbinomial(),
                                         chains = 4, cores = 4,
                                         backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q3, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q3", ".RDS"))

m_unadj_phq_2023_gam_sleep_q1 <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q1", ".RDS"))
m_unadj_phq_2023_gam_sleep_q2 <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q2", ".RDS"))
m_unadj_phq_2023_gam_sleep_q3 <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q3", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q1_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q1,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q1_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q1_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q2_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q2,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q2_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q2_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q3_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q3,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q3_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q3_99ci", ".RDS"))

## all stratified by insomnia ------------
m_unadj_phq_2023_gam_goodsleep_bl <- brmcoda(clr_acc_mhq_goodsleep_bl,
                                             phq_2023 ~ 
                                               s(ilr1) + s(ilr2) + s(ilr3),
                                             family = zero_inflated_negbinomial(),
                                             chains = 4, cores = 4,
                                             backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_goodsleep_bl, paste0(outputdir, "m_unadj_phq_2023_gam_goodsleep_bl", ".RDS"))

m_unadj_phq_2023_gam_insomnia_mild_bl <- brmcoda(clr_acc_mhq_insomnia_mild_bl,
                                                 phq_2023 ~ 
                                                   s(ilr1) + s(ilr2) + s(ilr3),
                                                 family = zero_inflated_negbinomial(),
                                                 chains = 4, cores = 4,
                                                 backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_insomnia_mild_bl, paste0(outputdir, "m_unadj_phq_2023_gam_insomnia_mild_bl", ".RDS"))

m_unadj_phq_2023_gam_insomnia_persistent_bl <- brmcoda(clr_acc_mhq_insomnia_persistent_bl,
                                                       phq_2023 ~ 
                                                         s(ilr1) + s(ilr2) + s(ilr3),
                                                       family = zero_inflated_negbinomial(),
                                                       chains = 4, cores = 4,
                                                       backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_insomnia_persistent_bl, paste0(outputdir, "m_unadj_phq_2023_gam_insomnia_persistent_bl", ".RDS"))

m_unadj_phq_2023_gam_goodsleep_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_goodsleep_bl", ".RDS"))
m_unadj_phq_2023_gam_insomnia_mild_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_insomnia_mild_bl", ".RDS"))
m_unadj_phq_2023_gam_insomnia_persistent_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_insomnia_persistent_bl", ".RDS"))

m_unadj_phq_2023_gam_sub_goodsleep_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_goodsleep_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_goodsleep_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_goodsleep_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_insomnia_mild_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_insomnia_mild_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_insomnia_mild_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_insomnia_mild_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_insomnia_persistent_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_insomnia_persistent_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_insomnia_persistent_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_insomnia_persistent_bl_99ci", ".RDS"))

## quantiles - good sleep baseline -----------------------
m_unadj_phq_2023_gam_sleep_q1_goodsleep_bl <- brmcoda(clr_acc_mhq_sleep_q1_goodsleep_bl,
                                                      phq_2023 ~ 
                                                        s(ilr1) + s(ilr2) + s(ilr3),
                                                      family = zero_inflated_negbinomial(),
                                                      chains = 4, cores = 4,
                                                      backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q1_goodsleep_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q1_goodsleep_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q2_goodsleep_bl <- brmcoda(clr_acc_mhq_sleep_q2_goodsleep_bl,
                                                      phq_2023 ~ 
                                                        s(ilr1) + s(ilr2) + s(ilr3),
                                                      family = zero_inflated_negbinomial(),
                                                      chains = 4, cores = 4,
                                                      backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q2_goodsleep_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q2_goodsleep_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q3_goodsleep_bl <- brmcoda(clr_acc_mhq_sleep_q3_goodsleep_bl,
                                                      phq_2023 ~ 
                                                        s(ilr1) + s(ilr2) + s(ilr3),
                                                      family = zero_inflated_negbinomial(),
                                                      chains = 4, cores = 4,
                                                      backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q3_goodsleep_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q3_goodsleep_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q1_goodsleep_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q1_goodsleep_bl", ".RDS"))
m_unadj_phq_2023_gam_sleep_q2_goodsleep_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q2_goodsleep_bl", ".RDS"))
m_unadj_phq_2023_gam_sleep_q3_goodsleep_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q3_goodsleep_bl", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q1_goodsleep_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q1_goodsleep_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q1_goodsleep_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q1_goodsleep_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q2_goodsleep_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q2_goodsleep_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q2_goodsleep_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q2_goodsleep_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q3_goodsleep_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q3_goodsleep_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q3_goodsleep_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q3_goodsleep_bl_99ci", ".RDS"))

## quantiles - mild insomnia baseline -----------------------
m_unadj_phq_2023_gam_sleep_q1_insomnia_mild_bl <- brmcoda(clr_acc_mhq_sleep_q1_insomnia_mild_bl,
                                                          phq_2023 ~ 
                                                            s(ilr1) + s(ilr2) + s(ilr3),
                                                          family = zero_inflated_negbinomial(),
                                                          chains = 4, cores = 4,
                                                          backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q1_insomnia_mild_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q1_insomnia_mild_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q2_insomnia_mild_bl <- brmcoda(clr_acc_mhq_sleep_q2_insomnia_mild_bl,
                                                          phq_2023 ~ 
                                                            s(ilr1) + s(ilr2) + s(ilr3),
                                                          family = zero_inflated_negbinomial(),
                                                          chains = 4, cores = 4,
                                                          backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q2_insomnia_mild_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q2_insomnia_mild_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q3_insomnia_mild_bl <- brmcoda(clr_acc_mhq_sleep_q3_insomnia_mild_bl,
                                                          phq_2023 ~ 
                                                            s(ilr1) + s(ilr2) + s(ilr3),
                                                          family = zero_inflated_negbinomial(),
                                                          chains = 4, cores = 4,
                                                          backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q3_insomnia_mild_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q3_insomnia_mild_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q1_insomnia_mild_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q1_insomnia_mild_bl", ".RDS"))
m_unadj_phq_2023_gam_sleep_q2_insomnia_mild_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q2_insomnia_mild_bl", ".RDS"))
m_unadj_phq_2023_gam_sleep_q3_insomnia_mild_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q3_insomnia_mild_bl", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q1_insomnia_mild_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q2_insomnia_mild_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q3_insomnia_mild_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci", ".RDS"))

## quantiles - persistent insomnia baseline -----------------------
m_unadj_phq_2023_gam_sleep_q1_insomnia_persistent_bl <- brmcoda(clr_acc_mhq_sleep_q1_insomnia_persistent_bl,
                                                                phq_2023 ~ 
                                                                  s(ilr1) + s(ilr2) + s(ilr3),
                                                                family = zero_inflated_negbinomial(),
                                                                chains = 4, cores = 4,
                                                                backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q1_insomnia_persistent_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q1_insomnia_persistent_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q2_insomnia_persistent_bl <- brmcoda(clr_acc_mhq_sleep_q2_insomnia_persistent_bl,
                                                                phq_2023 ~ 
                                                                  s(ilr1) + s(ilr2) + s(ilr3),
                                                                family = zero_inflated_negbinomial(),
                                                                chains = 4, cores = 4,
                                                                backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q2_insomnia_persistent_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q2_insomnia_persistent_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q3_insomnia_persistent_bl <- brmcoda(clr_acc_mhq_sleep_q3_insomnia_persistent_bl,
                                                                phq_2023 ~ 
                                                                  s(ilr1) + s(ilr2) + s(ilr3),
                                                                family = zero_inflated_negbinomial(),
                                                                chains = 4, cores = 4,
                                                                backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q3_insomnia_persistent_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q3_insomnia_persistent_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q1_insomnia_persistent_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q1_insomnia_persistent_bl", ".RDS"))
m_unadj_phq_2023_gam_sleep_q2_insomnia_persistent_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q2_insomnia_persistent_bl", ".RDS"))
m_unadj_phq_2023_gam_sleep_q3_insomnia_persistent_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q3_insomnia_persistent_bl", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q1_insomnia_persistent_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q2_insomnia_persistent_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q3_insomnia_persistent_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  scale = "response",
  cores = 4,
  ci = 0.995
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci", ".RDS"))

## quantiles - insomnia baseline -----------------------
m_unadj_phq_2023_gam_sleep_q1_insomnia_bl <- brmcoda(clr_acc_mhq_sleep_q1_insomnia_bl,
                                                   phq_2023 ~ 
                                                     s(ilr1) + s(ilr2) + s(ilr3),
                                                   family = zero_inflated_negbinomial(),
                                                   chains = 4, cores = 4,
                                                   backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q1_insomnia_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q1_insomnia_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q2_insomnia_bl <- brmcoda(clr_acc_mhq_sleep_q2_insomnia_bl,
                                                   phq_2023 ~ 
                                                     s(ilr1) + s(ilr2) + s(ilr3),
                                                   family = zero_inflated_negbinomial(),
                                                   chains = 4, cores = 4,
                                                   backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q2_insomnia_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q2_insomnia_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q3_insomnia_bl <- brmcoda(clr_acc_mhq_sleep_q3_insomnia_bl,
                                                   phq_2023 ~ 
                                                     s(ilr1) + s(ilr2) + s(ilr3),
                                                   family = zero_inflated_negbinomial(),
                                                   chains = 4, cores = 4,
                                                   backend = "cmdstanr"
)
saveRDS(m_unadj_phq_2023_gam_sleep_q3_insomnia_bl, paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q3_insomnia_bl", ".RDS"))

m_unadj_phq_2023_gam_sleep_q1_insomnia_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q1_insomnia_bl", ".RDS"))
m_unadj_phq_2023_gam_sleep_q2_insomnia_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q2_insomnia_bl", ".RDS"))
m_unadj_phq_2023_gam_sleep_q3_insomnia_bl <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sleep_q3_insomnia_bl", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q1_insomnia_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q2_insomnia_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_bl_99ci <- substitution(
  m_unadj_phq_2023_gam_sleep_q3_insomnia_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  scale = "response",
  cores = 4,
  ci = 0.995
)
saveRDS(m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_bl_99ci, paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_bl_99ci", ".RDS"))


## phq9 summary --------------------
m_unadj_phq_2023_gam_sub_sleep_q1_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q1_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_sleep_q2_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q2_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_sleep_q3_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q3_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_goodsleep_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_goodsleep_bl_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_insomnia_mild_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_insomnia_mild_bl_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_insomnia_persistent_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_insomnia_persistent_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q1_goodsleep_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q1_goodsleep_bl_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_sleep_q2_goodsleep_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q2_goodsleep_bl_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_sleep_q3_goodsleep_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q3_goodsleep_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci", ".RDS"))

m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_bl_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_bl_99ci", ".RDS"))
m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_bl_99ci", ".RDS"))

summary(m_unadj_phq_2023_gam_sub_sleep_q1_99ci, delta = 20)[Delta == 20]
summary(m_unadj_phq_2023_gam_sub_sleep_q2_99ci, delta = 20)[Delta == 20]
summary(m_unadj_phq_2023_gam_sub_sleep_q3_99ci, delta = 20)[Delta == 20]

summary(m_unadj_phq_2023_gam_sub_goodsleep_bl_99ci, delta = 20)
summary(m_unadj_phq_2023_gam_sub_insomnia_mild_bl_99ci, delta = 20)
summary(m_unadj_phq_2023_gam_sub_insomnia_persistent_bl_99ci, delta = 20)

# baseline insomnia
summary(m_unadj_phq_2023_gam_sub_sleep_q1_goodsleep_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_phq_2023_gam_sub_sleep_q2_goodsleep_bl_99ci, delta = 20)
summary(m_unadj_phq_2023_gam_sub_sleep_q3_goodsleep_bl_99ci, delta = 20)

summary(m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci, delta = 20)
summary(m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci, delta = 20)
summary(m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci, delta = 20)

summary(m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci, delta = 20)
summary(m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci, delta = 20)
summary(m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci, delta = 20)

summary(m_unadj_phq_2023_gam_sub_sleep_q1_insomnia_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_phq_2023_gam_sub_sleep_q2_insomnia_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_phq_2023_gam_sub_sleep_q3_insomnia_bl_99ci, delta = 20)[Delta == 20]

# GAD7 -------------------
## all stratified by sleep quantiles ------------
m_unadj_gad_2023_gam_sleep_q1 <- brmcoda(clr_acc_mhq_sleep_q1,
                                         gad_2023 ~ 
                                           s(ilr1) + s(ilr2) + s(ilr3),
                                         family = zero_inflated_negbinomial(),
                                         chains = 4, cores = 4,
                                         backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q1, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q1", ".RDS"))

m_unadj_gad_2023_gam_sleep_q2 <- brmcoda(clr_acc_mhq_sleep_q2,
                                         gad_2023 ~ 
                                           s(ilr1) + s(ilr2) + s(ilr3),
                                         family = zero_inflated_negbinomial(),
                                         chains = 4, cores = 4,
                                         backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q2, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q2", ".RDS"))

m_unadj_gad_2023_gam_sleep_q3 <- brmcoda(clr_acc_mhq_sleep_q3,
                                         gad_2023 ~ 
                                           s(ilr1) + s(ilr2) + s(ilr3),
                                         family = zero_inflated_negbinomial(),
                                         chains = 4, cores = 4,
                                         backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q3, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q3", ".RDS"))

m_unadj_gad_2023_gam_sleep_q1 <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q1", ".RDS"))
m_unadj_gad_2023_gam_sleep_q2 <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q2", ".RDS"))
m_unadj_gad_2023_gam_sleep_q3 <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q3", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q1_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q1,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q1_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q1_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q2_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q2,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q2_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q2_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q3_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q3,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q3_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q3_99ci", ".RDS"))

## all stratified by insomnia ------------
m_unadj_gad_2023_gam_goodsleep_bl <- brmcoda(clr_acc_mhq_goodsleep_bl,
                                             gad_2023 ~ 
                                               s(ilr1) + s(ilr2) + s(ilr3),
                                             family = zero_inflated_negbinomial(),
                                             chains = 4, cores = 4,
                                             backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_goodsleep_bl, paste0(outputdir, "m_unadj_gad_2023_gam_goodsleep_bl", ".RDS"))

m_unadj_gad_2023_gam_insomnia_mild_bl <- brmcoda(clr_acc_mhq_insomnia_mild_bl,
                                                 gad_2023 ~ 
                                                   s(ilr1) + s(ilr2) + s(ilr3),
                                                 family = zero_inflated_negbinomial(),
                                                 chains = 4, cores = 4,
                                                 backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_insomnia_mild_bl, paste0(outputdir, "m_unadj_gad_2023_gam_insomnia_mild_bl", ".RDS"))

m_unadj_gad_2023_gam_insomnia_persistent_bl <- brmcoda(clr_acc_mhq_insomnia_persistent_bl,
                                                       gad_2023 ~ 
                                                         s(ilr1) + s(ilr2) + s(ilr3),
                                                       family = zero_inflated_negbinomial(),
                                                       chains = 4, cores = 4,
                                                       backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_insomnia_persistent_bl, paste0(outputdir, "m_unadj_gad_2023_gam_insomnia_persistent_bl", ".RDS"))

m_unadj_gad_2023_gam_goodsleep_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_goodsleep_bl", ".RDS"))
m_unadj_gad_2023_gam_insomnia_mild_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_insomnia_mild_bl", ".RDS"))
m_unadj_gad_2023_gam_insomnia_persistent_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_insomnia_persistent_bl", ".RDS"))

m_unadj_gad_2023_gam_sub_goodsleep_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_goodsleep_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_goodsleep_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_goodsleep_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_insomnia_mild_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_insomnia_mild_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_insomnia_mild_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_insomnia_mild_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_insomnia_persistent_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_insomnia_persistent_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_insomnia_persistent_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_insomnia_persistent_bl_99ci", ".RDS"))

## quantiles - good sleep baseline -----------------------
m_unadj_gad_2023_gam_sleep_q1_goodsleep_bl <- brmcoda(clr_acc_mhq_sleep_q1_goodsleep_bl,
                                                      gad_2023 ~ 
                                                        s(ilr1) + s(ilr2) + s(ilr3),
                                                      family = zero_inflated_negbinomial(),
                                                      chains = 4, cores = 4,
                                                      backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q1_goodsleep_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q1_goodsleep_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q2_goodsleep_bl <- brmcoda(clr_acc_mhq_sleep_q2_goodsleep_bl,
                                                      gad_2023 ~ 
                                                        s(ilr1) + s(ilr2) + s(ilr3),
                                                      family = zero_inflated_negbinomial(),
                                                      chains = 4, cores = 4,
                                                      backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q2_goodsleep_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q2_goodsleep_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q3_goodsleep_bl <- brmcoda(clr_acc_mhq_sleep_q3_goodsleep_bl,
                                                      gad_2023 ~ 
                                                        s(ilr1) + s(ilr2) + s(ilr3),
                                                      family = zero_inflated_negbinomial(),
                                                      chains = 4, cores = 4,
                                                      backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q3_goodsleep_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q3_goodsleep_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q1_goodsleep_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q1_goodsleep_bl", ".RDS"))
m_unadj_gad_2023_gam_sleep_q2_goodsleep_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q2_goodsleep_bl", ".RDS"))
m_unadj_gad_2023_gam_sleep_q3_goodsleep_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q3_goodsleep_bl", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q1_goodsleep_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q1_goodsleep_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q1_goodsleep_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q1_goodsleep_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q2_goodsleep_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q2_goodsleep_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q2_goodsleep_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q2_goodsleep_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q3_goodsleep_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q3_goodsleep_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q3_goodsleep_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q3_goodsleep_bl_99ci", ".RDS"))

## quantiles - mild insomnia baseline -----------------------
m_unadj_gad_2023_gam_sleep_q1_insomnia_mild_bl <- brmcoda(clr_acc_mhq_sleep_q1_insomnia_mild_bl,
                                                          gad_2023 ~ 
                                                            s(ilr1) + s(ilr2) + s(ilr3),
                                                          family = zero_inflated_negbinomial(),
                                                          chains = 4, cores = 4,
                                                          backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q1_insomnia_mild_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q1_insomnia_mild_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q2_insomnia_mild_bl <- brmcoda(clr_acc_mhq_sleep_q2_insomnia_mild_bl,
                                                          gad_2023 ~ 
                                                            s(ilr1) + s(ilr2) + s(ilr3),
                                                          family = zero_inflated_negbinomial(),
                                                          chains = 4, cores = 4,
                                                          backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q2_insomnia_mild_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q2_insomnia_mild_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q3_insomnia_mild_bl <- brmcoda(clr_acc_mhq_sleep_q3_insomnia_mild_bl,
                                                          gad_2023 ~ 
                                                            s(ilr1) + s(ilr2) + s(ilr3),
                                                          family = zero_inflated_negbinomial(),
                                                          chains = 4, cores = 4,
                                                          backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q3_insomnia_mild_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q3_insomnia_mild_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q1_insomnia_mild_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q1_insomnia_mild_bl", ".RDS"))
m_unadj_gad_2023_gam_sleep_q2_insomnia_mild_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q2_insomnia_mild_bl", ".RDS"))
m_unadj_gad_2023_gam_sleep_q3_insomnia_mild_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q3_insomnia_mild_bl", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q1_insomnia_mild_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q2_insomnia_mild_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q3_insomnia_mild_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci", ".RDS"))

## quantiles - persistent insomnia baseline -----------------------
m_unadj_gad_2023_gam_sleep_q1_insomnia_persistent_bl <- brmcoda(clr_acc_mhq_sleep_q1_insomnia_persistent_bl,
                                                                gad_2023 ~ 
                                                                  s(ilr1) + s(ilr2) + s(ilr3),
                                                                family = zero_inflated_negbinomial(),
                                                                chains = 4, cores = 4,
                                                                backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q1_insomnia_persistent_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q1_insomnia_persistent_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q2_insomnia_persistent_bl <- brmcoda(clr_acc_mhq_sleep_q2_insomnia_persistent_bl,
                                                                gad_2023 ~ 
                                                                  s(ilr1) + s(ilr2) + s(ilr3),
                                                                family = zero_inflated_negbinomial(),
                                                                chains = 4, cores = 4,
                                                                backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q2_insomnia_persistent_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q2_insomnia_persistent_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q3_insomnia_persistent_bl <- brmcoda(clr_acc_mhq_sleep_q3_insomnia_persistent_bl,
                                                                gad_2023 ~ 
                                                                  s(ilr1) + s(ilr2) + s(ilr3),
                                                                family = zero_inflated_negbinomial(),
                                                                chains = 4, cores = 4,
                                                                backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q3_insomnia_persistent_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q3_insomnia_persistent_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q1_insomnia_persistent_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q1_insomnia_persistent_bl", ".RDS"))
m_unadj_gad_2023_gam_sleep_q2_insomnia_persistent_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q2_insomnia_persistent_bl", ".RDS"))
m_unadj_gad_2023_gam_sleep_q3_insomnia_persistent_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q3_insomnia_persistent_bl", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q1_insomnia_persistent_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q2_insomnia_persistent_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q3_insomnia_persistent_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  scale = "response",
  cores = 4,
  ci = 0.995
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci", ".RDS"))

## quantiles - insomnia baseline -----------------------
m_unadj_gad_2023_gam_sleep_q1_insomnia_bl <- brmcoda(clr_acc_mhq_sleep_q1_insomnia_bl,
                                                     gad_2023 ~ 
                                                       s(ilr1) + s(ilr2) + s(ilr3),
                                                     family = zero_inflated_negbinomial(),
                                                     chains = 4, cores = 4,
                                                     backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q1_insomnia_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q1_insomnia_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q2_insomnia_bl <- brmcoda(clr_acc_mhq_sleep_q2_insomnia_bl,
                                                     gad_2023 ~ 
                                                       s(ilr1) + s(ilr2) + s(ilr3),
                                                     family = zero_inflated_negbinomial(),
                                                     chains = 4, cores = 4,
                                                     backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q2_insomnia_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q2_insomnia_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q3_insomnia_bl <- brmcoda(clr_acc_mhq_sleep_q3_insomnia_bl,
                                                     gad_2023 ~ 
                                                       s(ilr1) + s(ilr2) + s(ilr3),
                                                     family = zero_inflated_negbinomial(),
                                                     chains = 4, cores = 4,
                                                     backend = "cmdstanr"
)
saveRDS(m_unadj_gad_2023_gam_sleep_q3_insomnia_bl, paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q3_insomnia_bl", ".RDS"))

m_unadj_gad_2023_gam_sleep_q1_insomnia_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q1_insomnia_bl", ".RDS"))
m_unadj_gad_2023_gam_sleep_q2_insomnia_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q2_insomnia_bl", ".RDS"))
m_unadj_gad_2023_gam_sleep_q3_insomnia_bl <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sleep_q3_insomnia_bl", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q1_insomnia_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q2_insomnia_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  cores = 4,
  ci = 0.999
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_bl_99ci <- substitution(
  m_unadj_gad_2023_gam_sleep_q3_insomnia_bl,
  delta = 1:20,
  ref = "grandmean",
  level = "aggregate",
  scale = "response",
  cores = 4,
  ci = 0.995
)
saveRDS(m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_bl_99ci, paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_bl_99ci", ".RDS"))

## gad summary --------------------
m_unadj_gad_2023_gam_sub_sleep_q1_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q1_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_sleep_q2_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q2_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_sleep_q3_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q3_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_goodsleep_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_goodsleep_bl_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_insomnia_mild_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_insomnia_mild_bl_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_insomnia_persistent_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_insomnia_persistent_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q1_goodsleep_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q1_goodsleep_bl_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_sleep_q2_goodsleep_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q2_goodsleep_bl_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_sleep_q3_goodsleep_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q3_goodsleep_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci", ".RDS"))

m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_bl_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_bl_99ci", ".RDS"))
m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_bl_99ci <- readRDS(paste0(outputdir, "m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_bl_99ci", ".RDS"))

summary(m_unadj_gad_2023_gam_sub_sleep_q1_99ci, delta = 20)[Delta == 20]
summary(m_unadj_gad_2023_gam_sub_sleep_q2_99ci, delta = 20)[Delta == 20]
summary(m_unadj_gad_2023_gam_sub_sleep_q3_99ci, delta = 20)[Delta == 20]

summary(m_unadj_gad_2023_gam_sub_goodsleep_bl_99ci, delta = 20)
summary(m_unadj_gad_2023_gam_sub_insomnia_mild_bl_99ci, delta = 20)
summary(m_unadj_gad_2023_gam_sub_insomnia_persistent_bl_99ci, delta = 20)

# baseline insomnia
summary(m_unadj_gad_2023_gam_sub_sleep_q1_goodsleep_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_gad_2023_gam_sub_sleep_q2_goodsleep_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_gad_2023_gam_sub_sleep_q3_goodsleep_bl_99ci, delta = 20)[Delta == 20]

summary(m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_mild_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_mild_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_mild_bl_99ci, delta = 20)[Delta == 20]

summary(m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_persistent_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_persistent_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_persistent_bl_99ci, delta = 20)[Delta == 20]

summary(m_unadj_gad_2023_gam_sub_sleep_q1_insomnia_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_gad_2023_gam_sub_sleep_q2_insomnia_bl_99ci, delta = 20)[Delta == 20]
summary(m_unadj_gad_2023_gam_sub_sleep_q3_insomnia_bl_99ci, delta = 20)[Delta == 20]
