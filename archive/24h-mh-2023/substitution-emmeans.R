...
#'                     if (summary) { # unadj OR adj averaging over reference grid
#'                       for (h in seq_len(nrow(d0))) {
#'                         if (object$model$family$family != "categorical") { # to be customised ot other families
#'                           
...
#'                           delta_y <- ysub - y0[, h]
#'                           
#'                         } else if (object$model$family$family == "categorical") {
#'                           
#'                           dsub <- cbind(dnew, ilrsub, refgrid[h, ])
#'                           resp <- object$model$formula$formula[[2]]
#'                           resp_level <- levels(as.factor(d0[, get(object$model$formula$formula[[2]])]))
#'                           
#'                           ysub <- fitted(
#'                             object,
#'                             newdata = dsub,
...
#'                           )
#'                           
#'                           ysub <- apply(ysub, 2, identity, simplify  = FALSE)
#'                           y0_h <- apply(y0, 2, identity, simplify = FALSE)
#'                           
#'                           delta_y <- mapply(function(a, b) a - b, ysub, y0_h[[h]])
#'                         }
#'                         hout[[h]] <- delta_y
#'                       }
#'                       
